...
public class CollectableSpawner : MonoBehaviour
{
    ...
    public GameObject[] penaltyCollectablePrefabs; // Array of penalty collectable prefabs
    ...
    public float penaltyCollectableSpawnInterval = 20f; // Time interval between penalty collectable spawns
    ...
    private void StartSpawning()
    {
        ...
        // Start spawning penalty collectables after yet another additional delay
        InvokeRepeating("SpawnPenaltyCollectable", initialDelay + penaltyCollectableSpawnInterval, penaltyCollectableSpawnInterval);
    }
    ...
    private void SpawnPenaltyCollectable()
    {
        // Randomly select a penalty collectable prefab
        GameObject collectablePrefab = penaltyCollectablePrefabs[Random.Range(0, penaltyCollectablePrefabs.Length)];

        // Calculate random position within spawn radius
        Vector2 randomPosition = (Random.insideUnitCircle * spawnRadius) + (Vector2)transform.position;

        // Instantiate penalty collectable at random position
        GameObject collectableInstance = Instantiate(collectablePrefab, randomPosition, Quaternion.identity);

        // Play sound effect for CPS reduction collectable
        audioManager.PlayPenaltyAppearClip(); // Assuming you have a method to play the sound effect in AudioManager
